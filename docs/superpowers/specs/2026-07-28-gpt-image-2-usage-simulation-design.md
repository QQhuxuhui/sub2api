...
| 比例 | 档 | low | medium | high | med/low | high/low |
|---|---|---|---|---|---|---|
| 1:1 | 1K | 196 | **1756** | **7024** | 8.959 | 35.837 |
| 1:1 | 2K | 397 | **3568** | — | 8.987 | — |
| 5:4 | 1K | 157 | **1370** | — | 8.726 | — |
| 4:3 | 1K | 144 | **1294** | — | 8.986 | — |
| 3:2 | 1K | 134 | **1167** | — | 8.709 | — |
| 16:9 | 1K | 106 | **947** | — | 8.934 | — |
| 16:9 | 4K | 371 | **3336** | **13342** | 8.992 | 35.962 |
| 21:9 | 1K | 82 | **733** | **2863** | 8.939 | 34.915 |

> **倍率不是常数**：med/low 在 8.709 ~ 8.992 之间（±1.6%），且与比例无稳定关系
> （5:4 是 8.726 而相邻的 4:3 是 8.986）。实测反例：21:9 的 1K high 推导值 2944，
> 实测 **2863**，偏差 −2.8%。
>
> 因此**推导格带 ±2~3% 计费误差**。既然目标是真实对齐官方，
> 剩余 26 格（medium 10 格、high 16 格）应在上线前补测实测值，
> 估算成本约 **$5.8**、约 26 次调用。补测脚本见实现计划「上线前置事项」。
> 在补测完成前，推导格用 `med = round(low × 8.87)`、`high = round(med × 4.0)`
> （4.0 由两处实测确认：7024/1756 = 4.000、13342/3336 = 3.999），并在表中标注。
...
6. `normalizeOpenAIImageQuality`：空/`auto`→low；low/medium/high 原样；
   未知值（如 `hd`、`4k`）→ false。
7. `openAIImageOutputTokens`：实测格精确；推导格等于约定公式；未知维度返回 false。
