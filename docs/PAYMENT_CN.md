...
> - 链上转账不可撤销，该服务商不支持网关退款，请保持退款开关关闭。管理员查单会先查网关订单状态，已支付时再读取收银台信息补齐金额。Epusdt 有两种情况不会给出链上哈希：查单接口从不返回；用户在收银台中途换了网络时，实际付的是子订单，而网关按父订单发回调，`block_transaction_id` 为空。这两种情况网关都已确认收款，订单照常入账，并以网关订单号（`epusdt-trade:<trade_id>`）占用防止重复入账；之后若收到带哈希的签名回调，会再补记该哈希。网关回调失败只重试 `order_notice_max_retry` 次，建议调大。
...
| 附近没有「无哈希入账」的订单 | 转账上链前 2 分钟至后 2 小时内，若有订单是无哈希入账且至今未补记哈希，拒绝并指出该订单——这笔转账可能就是它的付款（防止先留一张单不付、换网络付另一张、再拿同一笔转账来补前一张） |
