...
> - 链上转账不可撤销，该服务商不支持网关退款，请保持退款开关关闭。管理员查单会先查网关订单状态，已支付时再读取收银台信息补齐金额。
...
| 哈希未被使用过 | 网关回调入账的哈希、补单用过的哈希都不能再用 |
...
>
> 容差内的多付按订单全额入账，不会多给；本功能上线之前由网关回调入账的订单，审计日志里没有记录哈希，重复使用检查对它们不生效。
