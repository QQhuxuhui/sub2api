...
> §4 的横版/方图 54 格均为实测；竖版除 `720x1280` low 外按转置对称性推定，
> 推定值在本文中单独标明，不与实测混称。
...
下表 **54 格全部实测**，覆盖横版/方图 6 比例 × 3 尺寸档 × 3 quality。
...
竖版尺寸（4:5 / 3:4 / 2:3 / 9:16）暂按对应横版 token 使用；除上述
`720x1280` low 外，这些值尚未逐格实测。

**这 18 行覆盖 adobe2api `core/models/payloads.py::gpt_image_pixels_from_ratio` 的全部
30 组尺寸对应的比例/档位**（横竖按转置对称合并）。已脚本校验：那 30 组 100%
满足 §3 的官方约束；其中 12 个竖版尺寸的 token 仍需补测确认。
...
21:9 的 1K high 推得 2944，实测 **2863**（−2.8%）。§4 的横版/方图不再有此风险。
...
- §3 尺寸约束
- §4 表中横版/方图 **全部 54 格**（low / medium / high 三列均为实测）
- §5 倍率结构（由 §4 实测值直接计算）
- §6 公式（13 个实测点全中）
- §7 全部响应结构（实抓）

**推导值**：12 个竖版尺寸沿用横版/方图表的对应值；只有 `720x1280` low
完成直接对称性验证。实现必须把这部分视为有前提的推定，并在上线前补足交叉抽测。
