export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        openaiImageChromeImpersonationDesc: '全局开关。开启后，OpenAI OAuth/Codex 的生图请求（打到 chatgpt.com）将通过模拟 Chrome 的客户端发送（JA3/JA4 TLS + HTTP/2 指纹），用于绕过 Cloudflare 检测。不影响 API-key 生图（api.openai.com）及文本流量。默认关闭。',
      ...
      payment: {
        ...
        field_receiveAddresses: '可信收款地址（补单用）',
        field_epusdtReceiveAddressesHint: '可选，逗号分隔。按交易哈希补单时，除订单在网关登记的收款地址外，也认这些地址。用户在收银台中途切换过网络（如 TRON 换 BSC）时需要：把你在网关配置的各链收款地址都填上。',
        field_chainRpc: '链上节点（补单用）',
        field_epusdtChainRpcHint: '可选。补单核对默认使用公共节点；公共节点不稳定时可覆盖，格式：binance=https://节点地址,tron=https://节点地址（支持 tron / binance / polygon / ethereum，仅限 https）。',
