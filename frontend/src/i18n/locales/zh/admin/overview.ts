export default {
    ...
    users: {
      ...
      emptyResponseBilling: {
        menuItem: '空返回免单',
        title: '空返回不扣费规则',
        subtitle: '为用户 {email} 配置：命中规则的生图请求，若上游一张图都没返回，整笔记 $0 不扣费。',
        columns: {
          group: '分组',
          model: '模型',
          enabled: '启用',
        ...
        allGroups: '全部分组',
        allModels: '全部模型',
        modelPlaceholder: '留空 = 该分组下全部模型',
        notePlaceholder: '选填',
        addRule: '添加规则',
        remove: '删除',
        empty: '暂无规则。未配置规则时，空返回照常按原价计费。',
        hint: '分组与模型都留空 = 该用户全局生效。模型名不区分大小写，会同时匹配请求模型 / 计费模型 / 上游模型。仅对能观测到出图数的链路生效（Gemini 原生生图、OpenAI images）。',
        duplicate: '存在重复的 分组 × 模型 规则，请合并后再保存。',
        save: '保存',
        saving: '保存中...',
        cancel: '取消',
        updateSuccess: '空返回免单规则已更新，即时生效',
        updateFailed: '保存失败',
        loadFailed: '加载失败',
      }
    },
