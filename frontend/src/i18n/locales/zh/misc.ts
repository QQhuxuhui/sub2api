export default {
  ...
  payment: {
    ...
    createOrder: '确认支付',
    usdtCalc: {
      mustArrive: '请让到账金额正好是',
      waiting: '在收银台选好网络后，这里会显示金额',
      rule: '少 0.01 也不会自动到账；网络要和收银台选的一致。转完没到账，请带上交易哈希联系客服。',
      fromExchange: '从交易所提币？点这里算该填多少',
      fee: '交易所手续费',
      enter: '提币数量填',
      feeHint: '手续费在交易所的提币页面能看到。用自己的钱包转账不用算，直接转上面的金额。',
      copy: '复制',
    },
    usdtNotice: {
      title: 'USDT 转账请注意',
      exactAmount: '到账金额必须和收银台显示的完全一致，少 0.01 也不会自动到账。',
      exchangeFee: '从交易所提币会被扣手续费，下一步的页面会帮你算好该填多少。',
    },
    methods: {
      easypay: '易支付',
      alipay: '支付宝',
    ...
    errors: {
      INVALID_TX_HASH: '交易哈希格式不对，应为 64 位十六进制',
      UNSUPPORTED_NETWORK: '不支持该网络的链上核对',
      UNSUPPORTED_PROVIDER: '该支付服务商不支持按交易哈希补单',
      MISSING_TRADE_NO: '订单没有网关交易号，无法核对',
      ...
      FULFILLMENT_FAILED: '订单已标记为已支付，但入账失败，请在订单列表点“重试”',
    ...
    admin: {
      ...
      settleByTx: {
        action: '哈希补单',
        title: '按交易哈希补单',
        intro: '用于网关没有自动入账的 USDT 订单（常见原因：交易所提币扣了手续费，到账金额比订单少）。系统会到链上核对这笔交易，确认无误后按订单全额入账。',
        fraudWarning: '链上交易是公开的，任何人都能查到转入你收款地址的哈希。确认前请核对付款地址/提币记录确实属于该用户；多个站点共用同一网关钱包时，还要在网关后台确认这笔哈希没有挂在别的订单上。',
        txHash: '交易哈希（TxID）',
        txHashPlaceholder: '粘贴链上交易哈希，64 位十六进制，可带 0x',
        network: '网络',
        networkAuto: '自动识别',
        verify: '链上核对',
        confirm: '确认入账',
        ...
        blockTime: '上链时间',
        confirmations: '确认数',
        shortfallWarning: '到账比应付少 {shortfall} {token}，确认后仍按订单全额（{amount}）入账。',
        success: '补单成功，订单已入账',
      },
      approveRefund: '批准退款',
      retryRefund: '重试退款',
