export default {
  ...
  payment: {
    ...
    usdtCalc: {
      title: 'How much do I send?',
      intro: 'The amount that ARRIVES must match the cashier to the cent. Exchanges take their withdrawal fee out of the amount you enter, so you have to enter a bit more — worked out for you below.',
      source_exchange: 'Withdraw from an exchange',
      source_wallet: 'Send from my own wallet',
      arrive: 'Must arrive ({token})',
      arrivePlaceholder: 'Amount shown in the cashier',
      arriveFromGateway: 'Read automatically, network: {network}. Check it against the cashier; if they differ, the cashier is right.',
      arriveWaiting: 'Filled in automatically once you pick a network in the cashier, or type the amount the cashier shows.',
      fee: 'Exchange withdrawal fee ({token})',
      feePlaceholder: 'See your exchange withdrawal page',
      feeHint: 'Differs per exchange and network: usually about 1 on TRC20, far below 1 on BSC / Polygon. Your exchange withdrawal page has the real figure.',
      resultExchange: 'Enter this as the withdrawal amount:',
      resultWallet: 'Send exactly:',
      noteExchange: '{arrive} to arrive + {fee} fee. Before submitting, check the "receive amount" on the exchange page: it should be exactly {arrive} {token}.',
      noteWallet: 'The network fee is paid separately by your wallet in TRX / BNB etc.; it is not taken out of the amount.',
      needFee: 'Enter the exchange withdrawal fee to get the amount to type in.',
      copy: 'Copy',
      copied: 'Copied',
    },
    usdtNotice: {
    ...
    errors: {
      INVALID_TX_HASH: 'Malformed transaction hash: expected 64 hex characters',
      UNSUPPORTED_NETWORK: 'On-chain verification is not supported for this network',
      UNSUPPORTED_PROVIDER: 'This payment provider does not support settling by transaction hash',
      ...
      HASHLESS_PAYMENT_NEARBY: 'Order #{order_id} was credited around the time of this transfer without a transaction hash from the gateway, so this transfer may already have paid it. Check in the gateway admin which order holds this hash; if it really belongs to this order, adjust the balance manually.',
      ...
      FULFILLMENT_FAILED: 'The order was marked paid but crediting failed, use Retry in the order list',
    ...
    admin: {
      ...
      settleByTx: {
        action: 'Settle by hash',
        title: 'Settle by transaction hash',
        intro: 'For USDT orders the gateway did not credit on its own (typically an exchange withdrawal that lost a fee on the way and arrived short). The transaction is verified on chain, then the order is credited in full.',
        fraudWarning: 'On-chain transactions are public: anyone can look up a hash that paid your address. Before confirming, check that the sender or withdrawal record really belongs to this user; when several sites share one gateway wallet, also make sure the gateway has not attached this hash to another order.',
        txHash: 'Transaction hash (TxID)',
        txHashPlaceholder: 'Paste the on-chain hash: 64 hex characters, 0x optional',
        network: 'Network',
        networkAuto: 'Detect automatically',
        verify: 'Verify on chain',
        confirm: 'Confirm and credit',
        verified: 'Verified on chain',
        received: 'Received',
        expected: 'Order expects',
        from: 'Sender',
        to: 'Recipient',
        blockTime: 'Block time',
        confirmations: 'Confirmations',
        shortfallWarning: 'Received {shortfall} {token} less than expected; the order will still be credited in full ({amount}).',
        success: 'Order settled and credited',
      },
