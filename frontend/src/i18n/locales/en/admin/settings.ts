export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        openaiImageChromeImpersonationDesc: 'Global switch. When on, OpenAI OAuth/Codex image-generation requests (to chatgpt.com) are sent through a Chrome-impersonating client (JA3/JA4 TLS + HTTP/2 fingerprint) to bypass Cloudflare checks. Does not affect API-key image requests (api.openai.com) or text traffic. Default off.',
      ...
      payment: {
        ...
        field_receiveAddresses: 'Trusted receiving addresses (manual settlement)',
        field_epusdtReceiveAddressesHint: 'Optional, comma separated. When settling an order by transaction hash these addresses are accepted in addition to the one the gateway recorded for the order. Needed when the payer switched network inside the cashier (e.g. TRON to BSC): list the receiving addresses you configured on the gateway for every chain.',
        field_chainRpc: 'Chain RPC endpoints (manual settlement)',
        field_epusdtChainRpcHint: 'Optional. Settlement verification uses public RPC nodes by default; override them if they are unreliable: binance=https://node,tron=https://node (tron / binance / polygon / ethereum, https only).',
        ...
        field_epusdtChainHint: 'Fill both token and network to pin the receiving chain (e.g. usdt / tron, usdt / binance; use the network id returned by the gateway /payments/gmpay/v1/config — BSC is "binance"); leave both empty to let the payer pick any chain enabled on the gateway. Filling only one is rejected.',
        epusdtGuideSummary: 'Self-hosted Epusdt (GM Pay) crypto gateway: orders are created with a fiat amount, the payer is redirected to the gateway cashier to transfer, and the gateway calls Sub2API back once the on-chain payment lands.',
        epusdtGuideNote: 'The notify and return URLs are submitted with every order, so nothing needs configuring in the Epusdt admin; the gateway must be able to reach this site\'s webhook URL. On-chain transfers are irreversible and this provider has no gateway refund, so keep refunds disabled.',
