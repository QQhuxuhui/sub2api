...
const {
  ...
  updateModelMappingTemplate: vi.fn(),
  getGroups: vi.fn(),
  listProxies: vi.fn(),
  getProviders: vi.fn(),
...
beforeEach(() => {
  getModelMappingTemplate.mockReset().mockResolvedValue({});
  updateModelMappingTemplate
    .mockReset()
    .mockImplementation(async (_platform, mapping) => mapping);
});

describe("admin SettingsView model mapping template", () => {
  beforeEach(() => {
    getSettings.mockReset().mockResolvedValue({ ...baseSettingsResponse });
    getWebSearchEmulationConfig.mockReset().mockResolvedValue({ enabled: false, providers: [] });
    getAdminApiKey.mockReset().mockResolvedValue({ exists: false, masked_key: "" });
    ...
    listProxies.mockReset().mockResolvedValue({ items: [] });
    getProviders.mockReset().mockResolvedValue({ data: [] });
    getModelMappingTemplate.mockReset();
    updateModelMappingTemplate.mockReset();
    showError.mockReset();
  });

  it("ignores a stale response after switching platforms", async () => {
    const antigravity = deferred<Record<string, string>>();
    const openai = deferred<Record<string, string>>();
    getModelMappingTemplate.mockImplementation((platform: string) =>
      platform === "antigravity" ? antigravity.promise : openai.promise,
    );

    const wrapper = mountView();
    await flushPromises();
    await wrapper.get('[data-testid="model-mapping-template-platform"]').setValue("openai");
    openai.resolve({ "gpt-current": "gpt-target" });
    await flushPromises();
    antigravity.resolve({ stale: "mapping" });
    await flushPromises();

    expect((wrapper.get('[data-testid="model-mapping-template-json"]').element as HTMLTextAreaElement).value)
      .toContain("gpt-current");
    expect((wrapper.get('[data-testid="model-mapping-template-json"]').element as HTMLTextAreaElement).value)
      .not.toContain("stale");
  });

  it("keeps the current editor content and disables save when loading fails", async () => {
    getModelMappingTemplate.mockResolvedValueOnce({ existing: "mapping" });
    const wrapper = mountView();
    await flushPromises();
    getModelMappingTemplate.mockRejectedValueOnce(new Error("network"));
    ...
    expect((wrapper.get('[data-testid="model-mapping-template-json"]').element as HTMLTextAreaElement).value)
      .toContain("existing");
    expect(wrapper.get('[data-testid="model-mapping-template-save"]').attributes("disabled")).toBeDefined();
    expect(wrapper.text()).toContain("admin.settings.modelMappingTemplate.loadFailed");
  });

  it("allows correcting invalid JSON after a validation error", async () => {
    getModelMappingTemplate.mockResolvedValueOnce({});
    updateModelMappingTemplate.mockResolvedValueOnce({ valid: "mapping" });
    const wrapper = mountView();
    await flushPromises();

    const editor = wrapper.get('[data-testid="model-mapping-template-json"]');
    const saveButton = wrapper.get('[data-testid="model-mapping-template-save"]');
    await editor.setValue("{");
    await saveButton.trigger("click");

    expect(saveButton.attributes("disabled")).toBeUndefined();

    await editor.setValue('{"valid":"mapping"}');
    await saveButton.trigger("click");
    await flushPromises();
