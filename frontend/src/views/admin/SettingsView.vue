            ...
            <div class="space-y-5 p-6">
              <div class="flex items-center gap-3">
                ...
                <select
                  ...
                  data-testid="model-mapping-template-platform"
                  class="input w-48"
                >
              ...
              <template v-else>
                <div>
                  ...
                  <textarea
                    ...
                    rows="10"
                    spellcheck="false"
                    data-testid="model-mapping-template-json"
                ...
                >
                  <button
                    ...
                    :disabled="
                      ...
                      modelMappingTemplateLoading ||
                      modelMappingTemplateLoadFailed
                    "
                    data-testid="model-mapping-template-save"
                    class="btn btn-primary btn-sm"
                  >
                    {{
                      modelMappingTemplateSaving
...
const modelMappingTemplateLoadFailed = ref(false);
let modelMappingTemplateLoadRequestId = 0;
...
async function loadModelMappingTemplate(platform: PlatformType) {
  const requestId = ++modelMappingTemplateLoadRequestId;
  modelMappingTemplateLoading.value = true;
  modelMappingTemplateError.value = "";
  modelMappingTemplateLoadFailed.value = false;
  try {
    const mapping = await adminAPI.settings.getModelMappingTemplate(platform);
    if (
      requestId !== modelMappingTemplateLoadRequestId ||
      platform !== modelMappingTemplatePlatform.value
    ) {
      return;
    }
    ...
    const message = extractApiErrorMessage(
      error,
      t("admin.settings.modelMappingTemplate.loadFailed"),
    );
    modelMappingTemplateError.value = message;
    modelMappingTemplateLoadFailed.value = true;
    appStore.showError(message);
  } finally {
    if (requestId === modelMappingTemplateLoadRequestId) {
...
async function saveModelMappingTemplate() {
  if (modelMappingTemplateLoadFailed.value) {
    return;
  }
  ...
  const platform = modelMappingTemplatePlatform.value;
  try {
    const updated = await adminAPI.settings.updateModelMappingTemplate(
      platform,
      parsed,
    );
    if (platform === modelMappingTemplatePlatform.value) {
