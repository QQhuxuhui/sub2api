    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        ...
        <button
          ...
          :disabled="submitting || templateApplyBlocked"
...
watch(
  ...
  async ([on, mode, platform], _previous, onCleanup) => {
    ...
    if (on && mode === 'apply' && platform) {
      ...
      try {
        const mapping = await adminAPI.settings.getModelMappingTemplate(
          platform as PlatformType
        )
        if (cancelled) return
        templateMapping.value = mapping
      } catch (error: unknown) {
        if (cancelled) return
        templateMapping.value = {}
        templateError.value = extractApiErrorMessage(error, t('common.error'))
        appStore.showError(templateError.value)
      } finally {
        if (!cancelled) {
...
const buildUpdatePayload = (): Record<string, unknown> | null => {
  ...
  if (enableApplyTemplate.value && singleSelectedPlatform.value) {
    if (applyTemplateMode.value === 'clear') {
      ...
      credentialsChanged = true
    } else if (!templateLoading.value && !templateError.value) {
...
const handleSubmit = async () => {
  ...
  if (enableApplyTemplate.value && applyTemplateMode.value === 'apply' && templateError.value) {
    appStore.showError(templateError.value)
    return
  }
