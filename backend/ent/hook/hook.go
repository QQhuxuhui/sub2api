...
// The IntentRouterFunc type is an adapter to allow the use of ordinary
// function as IntentRouter mutator.
type IntentRouterFunc func(context.Context, *ent.IntentRouterMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f IntentRouterFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.IntentRouterMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.IntentRouterMutation", m)
}
