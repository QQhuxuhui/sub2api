...
// The IntentRouterFunc type is an adapter to allow the use of ordinary function as a Querier.
type IntentRouterFunc func(context.Context, *ent.IntentRouterQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f IntentRouterFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.IntentRouterQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.IntentRouterQuery", q)
}

// The TraverseIntentRouter type is an adapter to allow the use of ordinary function as Traverser.
type TraverseIntentRouter func(context.Context, *ent.IntentRouterQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseIntentRouter) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseIntentRouter) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.IntentRouterQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.IntentRouterQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.IntentRouterQuery:
		return &query[*ent.IntentRouterQuery, predicate.IntentRouter, intentrouter.OrderOption]{typ: ent.TypeIntentRouter, tq: q}, nil
