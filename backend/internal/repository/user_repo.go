...
func (r *userRepository) Update(ctx context.Context, userIn *service.User, fields service.UserUpdateFields) error {
	...
	// 采用上游 v0.1.172 的字段掩码写法（OAuth pending 账号接管 / lost-update 安全修复）：
	// 只写显式声明的列，未声明列保持库中现值，陈旧快照不会回滚并发的扣费/封禁/限额/授权。
	// 注意：上游同时移除了 SetBalance / SetTotalRecharged —— 余额与累计充值改由 AdjustBalance /
	// SetBalance / DeductBalance 等原子方法维护，此处不再写，切勿回退。
	...
	// dev 定制：用户级请求整体超时（request_timeout_seconds）。已纳入 UserUpdateFields 掩码，
	// 遵循上游 lost-update 安全修复——仅在管理端显式修改超时时写该列，不再无条件回写。
	if fields.RequestTimeoutSeconds {
		updateOp = updateOp.SetRequestTimeoutSeconds(userIn.RequestTimeoutSeconds)
	}
