...
func TestOpenAIGatewayServiceForwardImages_OAuth429CarriesSameAccountRetryWindow(t *testing.T) {
	...
	// 本仓的短窗口 429 原地自愈（doImageUpstreamWithRateLimitRetry）会先按 Retry-After 原地重试
	// 最多 openAIImageRateLimitMaxRetries 次再交回故障转移，重试窗口 deadline 相应后移；
	// 容差按原地重试的等待上限放宽，仍钉住「deadline 以故障转移时刻起算一个窗口」这一语义。
	inPlaceRetryBudget := time.Duration(openAIImageRateLimitMaxRetries)*openAIImageRateLimitMaxWait + time.Second
	require.WithinDuration(t, startedAt.Add(openAIOAuth429RetryWindow), failoverErr.SameAccountRetryDeadline, inPlaceRetryBudget)
	require.False(t, failoverErr.SameAccountRetryDeadline.Before(startedAt.Add(openAIOAuth429RetryWindow)))
}
