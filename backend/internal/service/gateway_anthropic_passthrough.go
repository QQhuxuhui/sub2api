...
func detectDisguisedClaudeError(body []byte) (bool, string) {
	...
	case "message":
		...
		if id == "" && model == "" &&
			...
			message := strings.TrimSpace(root.Get("content.0.text").String())
			if message == "" {
				message = "upstream returned empty message with no id/model"
			}
			return true, message
		}
	}
...
func (s *GatewayService) claudeErrorBodyIn2xxFailover(
	...
	isError, rawMessage := detectDisguisedClaudeError(body)
	if !isError {
		return nil
	}
...
func (s *GatewayService) handleNonStreamingResponseAnthropicAPIKeyPassthrough(
	...
	if failoverErr := s.claudeErrorBodyIn2xxFailover(ctx, resp, c, account, body, true); failoverErr != nil {
