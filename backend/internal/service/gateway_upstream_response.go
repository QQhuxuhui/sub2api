...
func (s *GatewayService) handleStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel string, mimicClaudeCode bool) (*streamingResult, error) {
	...
	usage := &ClaudeUsage{}
	var envelopeNormalizer *anthropicnorm.Normalizer
	if value, exists := c.Get(NormalizeEnvelopeContextKey); exists {
		if enabled, _ := value.(bool); enabled {
			envelopeNormalizer = anthropicnorm.For(account, true)
		}
	}
	...
	processSSEEvent := func(lines []string) ([]string, string, *sseUsagePatch, error) {
		...
		if envelopeNormalizer != nil && (eventType == "message_start" || eventType == "message_delta") {
			source := []byte(dataLine)
			if eventChanged {
				if changedData, marshalErr := json.Marshal(event); marshalErr == nil {
					source = changedData
				}
			}
			if rewritten, injectPing := envelopeNormalizer.RewriteStreamEvent(eventType, source); rewritten != nil {
				block := ""
				if eventName != "" {
					block = "event: " + eventName + "\n"
				}
...
func (s *GatewayService) handleNonStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, originalModel, mappedModel string) (*ClaudeUsage, error) {
	...
	if failoverErr := s.claudeErrorBodyIn2xxFailover(ctx, resp, c, account, body, false, mappedModel); failoverErr != nil {
	...
	if value, exists := c.Get(NormalizeEnvelopeContextKey); exists {
		if enabled, _ := value.(bool); enabled {
			if normalizer := anthropicnorm.For(account, true); normalizer != nil {
				body = normalizer.RewriteNonStreamingBody(body)
			}
		}
	}
