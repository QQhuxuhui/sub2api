	...
	for urlIdx, baseURL := range availableURLs {
		...
		for attempt := 1; attempt <= maxAttempts; attempt++ {
			...
			if resp.StatusCode >= 400 {
				...
				_ = resp.Body.Close()
				isAntigravityOAuth401 := resp.StatusCode == http.StatusUnauthorized &&
					p.account != nil && p.account.Platform == PlatformAntigravity && p.account.IsOAuth()
				if isAntigravityOAuth401 && !authRetried && s.tokenProvider != nil {
					authRetried = true
				...
				// Antigravity OAuth 401 在请求路径上先强制刷新；即使刷新失败也不应
				// 被自定义临时不可调度规则下线，后续统一由 HandleUpstreamError 仅失效缓存。
				if !isAntigravityOAuth401 {
					// ★ 统一入口：自定义错误码 + 临时不可调度
	...
	for _, d := range details {
		...
		if atType == googleRPCTypeErrorInfo {
			...
			if reason, ok := dm["reason"].(string); ok {
				...
				if reason == googleRPCReasonRateLimitExceeded || reason == googleRPCReasonQuotaExhausted {
