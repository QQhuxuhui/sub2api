...
type OpenAIAccountScheduleRequest struct {
	...
	// RequireImagesHighRes 是本仓自研：显式 2K/4K 尺寸只调度带 openai_images_highres 标记的账号。
...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatibleReason(ctx context.Context, account *Account, req OpenAIAccountScheduleRequest) (bool, string) {
	...
	if s != nil && s.service != nil && s.service.isImageOrgBlockedForSelection(account, req.RequiredImageCapability) {
		return false, "image_org_cooldown"
	}
	...
	if !accountSupportsOpenAICapabilities(account, req.RequiredCapability, req.RequiredImageCapability, req.RequireImagesHighRes) {
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForImages(
	...
	selection, decision, err := s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", requiredCapability, requireImagesHighRes, false, PlatformOpenAI, false, false)
	if err == nil && selection != nil && selection.Account != nil {
		return selection, decision, nil
	}
	// 如果要求 native 能力（如指定了模型）但没有可用的 APIKey 账号，回退到 basic（OAuth 账号）
	if requiredCapability == OpenAIImagesCapabilityNative {
		return s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", OpenAIImagesCapabilityBasic, requireImagesHighRes, false, PlatformOpenAI, false, false)
	}
...
func (s *OpenAIGatewayService) selectAccountWithScheduler(
	...
	selection, decision, err := s.selectAccountWithSchedulerOnce(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireImagesHighRes, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
	...
	return s.selectAccountWithSchedulerOnce(withOpenAIProxyStreamQuarantineBypass(ctx), groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireImagesHighRes, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
}
