...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, requestedModel ...string) (shouldDisable bool) {
	...
	case 401:
		...
		if authAccount.Platform == PlatformAntigravity {
			if authAccount.IsOAuth() && s.tokenCacheInvalidator != nil {
				if err := s.tokenCacheInvalidator.InvalidateToken(ctx, authAccount); err != nil {
					slog.Warn("antigravity_401_invalidate_cache_failed", "account_id", authAccount.ID, "error", err)
				}
			}
			slog.Info("antigravity_401_kept_schedulable", "account_id", authAccount.ID)
			return false
		}
