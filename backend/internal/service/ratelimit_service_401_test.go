...
func TestRateLimitService_HandleUpstreamError_OAuth401NoRefreshTokenSetsError(t *testing.T) {
	...
	t.Run("antigravity_no_refresh_token_stays_schedulable", func(t *testing.T) {
		repo := &rateLimitAccountRepoStub{}
		invalidator := &tokenCacheInvalidatorRecorder{}
		service := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
		service.SetTokenCacheInvalidator(invalidator)
		...
		require.False(t, shouldDisable)
		require.Equal(t, 0, repo.setErrorCalls, "Antigravity 401 must never sideline the account")
		require.Equal(t, 0, repo.tempCalls)
		require.Len(t, invalidator.accounts, 1)
	})
}
